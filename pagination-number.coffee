...
pagination =({ total:t ,prev:p ,next:n ,first:f ,last:l ,number:m,class:c }) ->
    ...
    if _.isObject(dp) 
        ...
        if dian.data.view.list and dp.total isnt 0
        ...
        if dian.data.view.list
            if dp.current_number isnt dp.total and dp.total isnt 0 and l isnt null and l isnt "" and l? then write( initLast(l) )
